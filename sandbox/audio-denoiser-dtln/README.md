...
### 1. Fine-tune DTLN with OPS-SAT Data (Recommended)
- **Use existing DTLN architecture** but retrain/fine-tune on OPS-SAT samples
- **Need pairs of (noisy OPS-SAT, clean OPS-SAT)** for training
- **Leverages proven DTLN architecture** for radio-specific domain
- **Training data available**: Clean voice recording for creating synthetic noisy pairs
- **Implementation**: Use cloned DTLN repo Python training pipeline with OPS-SAT dataset

### 2. Try Other Pre-trained Models
- **Facebook Demucs**: Music source separation, may handle RF interference better
- **Microsoft DNS Challenge models**: Speech enhancement optimized for various noise types
- **RNNoise**: Lightweight real-time speech denoising, good for embedded deployment
- **Evaluation approach**: Test each model on OPS-SAT samples using their respective APIs

### 3. Classical Signal Processing
- **Spectral subtraction**: Estimate noise spectrum, subtract from noisy signal
- **Wiener filtering**: Optimal linear filter based on signal/noise statistics
- **Adaptive noise cancellation**: Real-time adaptation to changing noise characteristics
- **Advantage**: No training data required, tunable parameters for radio-specific noise
