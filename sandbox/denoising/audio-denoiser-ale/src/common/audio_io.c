...
// Maximum expected resampling ratio for buffer allocation
// Assumes worst-case upsampling of 2x (e.g., 8kHz -> 16kHz)
#define MAX_RESAMPLE_RATIO 2
 ...
 *
 * Averaging prevents information loss and phase cancellation issues that
 * can occur when discarding one channel. This is the standard approach for
 * stereo-to-mono conversion in audio processing.
 *
 * Parameters:
 *   stereo_input - Interleaved stereo samples [L, R, L, R, ...]
 *   mono_output  - Output mono samples
 *   frames       - Number of stereo frames (pairs of samples)
 */
...
AudioData* load_audio(const char* filename) {
    ...
    if (sf_info.samplerate != TARGET_SAMPLE_RATE) {
        // Allocate buffer for worst-case upsampling (e.g., 8kHz -> 16kHz)
        final_data = malloc(sf_info.frames * MAX_RESAMPLE_RATIO * sizeof(float));
