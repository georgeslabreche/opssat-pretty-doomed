...
For the zenith snapshot (19:25:11), tracked in detail: carrier at **1295.992 MHz** (-8.1 kHz from 1296.000), +22.7 dB over the floor, -3 dB width 0.61 kHz, drifting -353 Hz/s (-636 Hz across carrier-present windows). These come straight from the per-snapshot `carrier.txt` / `carrier.json` in `carrier-analysis/` alongside this debrief, produced by [`detect_carrier.py`](../../../../../tools/src/detect_carrier.py); the cross-snapshot comparison is `carriers_summary.txt` / `.json` (plot: `carriers_summary.svg`), from [`summarize_carriers.py`](../../../../../tools/src/summarize_carriers.py).
...
Analysis produced on the ground from the raw IQ with [`detect_carrier.py`](../../../../../tools/src/detect_carrier.py), [`summarize_carriers.py`](../../../../../tools/src/summarize_carriers.py), and [`demod_audio.py`](../../../../../tools/src/demod_audio.py), and the on-board preview with `preview_onboard` (see [`docs/ONBOARD_PREVIEW.md`](../../../ONBOARD_PREVIEW.md)). The raw flight record (recording metadata and the minimally processed `run-05-raw-audio.zip`) is in the data folder: [`../../data/run-05-2026-07-03/`](../../data/run-05-2026-07-03/). The processed and enhanced audio for all six snapshots, with per-folder notes, is in [`run-05-audio-processing.zip`](run-05-audio-processing.zip) alongside this debrief.
