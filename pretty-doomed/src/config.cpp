...
static void apply_config_entry(const std::string& key, const std::string& value,
    ...
    else if (key == "sdr_enable_psd") cfg.sdr_enable_psd = (value == "true" || value == "1");
    else if (key == "sdr_keep_sc16") {
        if (value == "true" || value == "1") cfg.sdr_keep_sc16 = Sc16Keep::Always;
        else if (value == "detected") cfg.sdr_keep_sc16 = Sc16Keep::Detected;
        else cfg.sdr_keep_sc16 = Sc16Keep::Never;
    }
