 ...
 *
 * Generates a DOOM-themed postcard from run artifacts.
 * Uses stb single-header libraries (public domain, no external dependencies).
 */
...
#include <chrono>
#include "postcard.h"
#include "postcard_image.h"
#include "postcard_sc16.h"
...
bool generate_postcard(const PostcardArgs& args) {
    ...
    log_info() << "Generating postcard (scale=" << args.scale << ")...\n";
    log_info() << "  Frame: " << args.frame_path << "\n";
    if (!args.sc16_path.empty()) {
        log_info() << "  SC16:  " << args.sc16_path << "\n";
    } else {
        log_info() << "  SC16:  none (scatter/spectrogram skipped)\n";
    }

    srand((unsigned)time(nullptr) ^ (unsigned)getpid());

    // Load images
    Image frame = !args.frame_path.empty() ? load_image(args.frame_path.c_str()) : Image();
    Image logo_esa = !args.logo_esa.empty() ? load_image(args.logo_esa.c_str()) : Image();
    Image logo_doom = !args.logo_doom.empty() ? load_image(args.logo_doom.c_str()) : Image();
    Image logo_pretty = !args.logo_pretty.empty() ? load_image(args.logo_pretty.c_str()) : Image();

    if (frame.w == 0) {
        log_warning() << "Postcard: no DOOM frame loaded\n";
        return false;
    }

    // Layout (all dimensions scaled by scale factor)
    const int S = std::max(1, args.scale);
    const int S2 = 2 * S;
    const int PAD = 20 * S;
    ...
    // Timestamp passed directly by caller
    const std::string& timestamp = args.timestamp;
    ...
    if (!args.demo_name.empty()) bottom_lines++;
    ...
    for (int try_scale = 7 * S; try_scale >= S; try_scale -= S) {
        ...
        auto [w, nl] = wrap_text(args.transcription, try_scale, 0);
    ...
    if (best_wrapped.empty() && !args.transcription.empty()) {
        int lh = (GLYPH_H + 3) * S;
        int max_lines = std::max(1, transcript_avail_h / lh);
        auto [w, nl] = wrap_text(args.transcription, S, max_lines);
        best_scale = S;
        best_wrapped = w;
    }
    ...
    if (!args.sc16_path.empty()) {
        render_iq_scatter(canvas, PAD, body_y, body_w, body_h,
            args.sc16_path.c_str(), 120000, S);
    }
    ...
    for (int i = 0; i < S2; i++) {
        for (int x = frame_x - i - 1; x <= frame_x + frame_scaled.w + i; x++) {
            canvas.set_pixel(x, frame_y - i - 1, DOOM_FRAME_R, DOOM_FRAME_G, DOOM_FRAME_B);
            canvas.set_pixel(x, frame_y + frame_scaled.h + i, DOOM_FRAME_R, DOOM_FRAME_G, DOOM_FRAME_B);
        }
        for (int y = frame_y - i - 1; y <= frame_y + frame_scaled.h + i; y++) {
            canvas.set_pixel(frame_x - i - 1, y, DOOM_FRAME_R, DOOM_FRAME_G, DOOM_FRAME_B);
            canvas.set_pixel(frame_x + frame_scaled.w + i, y, DOOM_FRAME_R, DOOM_FRAME_G, DOOM_FRAME_B);
        }
    }
    ...
    canvas.fill_rect(PAD, text_y, canvas_w - PAD * 2, actual_text_h,
        DOOM_TXT_R, DOOM_TXT_G, DOOM_TXT_B);
    if (!args.sc16_path.empty()) {
        render_spectrogram_bg(canvas, PAD, text_y,
            canvas_w - PAD * 2, actual_text_h, args.sc16_path.c_str());
    }
    ...
    std::string ts_url = timestamp.empty()
        ? "OPSSAT.ESA.INT"
        : "COMMAND DETECTED AT " + timestamp + "  |  OPSSAT.ESA.INT";
    canvas.draw_text(tx, row, ts_url.c_str(),
        DOOM_WARM_R, DOOM_WARM_G, DOOM_WARM_B, S2);
    row -= small_lh + line_gap;

    if (!args.demo_name.empty()) {
        std::string line = "PLAY: " + args.demo_name +
            "  |  VOICE-COMMANDED FROM EARTH TO RUN DOOM IN SPACE";
    ...
    // Write output
    bool ok;
    std::string out_path = args.output_path;
    if (out_path.size() > 4 && out_path.substr(out_path.size() - 4) == ".png") {
        ok = stbi_write_png(args.output_path.c_str(), canvas.w, canvas.h, 4,
            canvas.pixels.data(), canvas.w * 4);
    } else {
        ok = stbi_write_jpg(args.output_path.c_str(), canvas.w, canvas.h, 4,
            canvas.pixels.data(), 92);
    }
    ...
    if (!ok) {
        log_warning() << "Postcard: failed to write " << args.output_path << "\n";
        return false;
    }

    log_info() << "Postcard: " << args.output_path
               << " (" << canvas.w << "x" << canvas.h << ")"
               << ", " << elapsed << "s\n";
    return true;
}
