...
bool run_capture(const PipelineConfig& cfg,
    ...
    unsigned long effective_rate = cfg.sdr_rate / cfg.sdr_decimation;
    if (effective_rate == 0) {
    ...
    // Design filters
    if (cfg.sdr_lpf_cutoff <= 0 || cfg.sdr_lpf_cutoff >= cfg.sdr_rate / 2.0) {
        log_error() << "LPF cutoff out of range\n";
        return false;
    }
    std::vector<float> lpf_taps = gr::filter::firdes::low_pass(
        1.0, cfg.sdr_rate, cfg.sdr_lpf_cutoff, cfg.sdr_lpf_transition,
